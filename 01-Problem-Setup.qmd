...
Where $f(Age, BMI)$ is a machine learning model that takes in the predictors $Age$, $BMI$, and make a prediction on one's $MeanBloodPressure$. The predictor's value is almost always known, and the response is not, which gives us our problem making sound predictions. The process of machine learning is figuring out the appropriate function $f()$. For example,
...
$$
MeanBloodPressure=20 + 3 \cdot Age - .2 \cdot BMI = 103.2
$$
...
2.  **Inference (Secondary in this course):** Which predictors are associated with the response, and how strong is the association?
    ...
    -   Prediction model example: Suppose the model is described as $MeanBloodPressure = f(Age,BMI)=20 + 3 \cdot Age - .2 \cdot BMI$. Each variable has a relationship to the outcome: an increase of $Age$ by 1 will lead to an increase of $MeanBloodPressure$ by 3. This measures the strength of association between a variable and the outcome.
...
Below, we split the entire data randomlly into Training and Testing sets, giving 80% of the data to Training, and 20% of the data to Testing. There are other ways of splitting data to consider, in scenarios such as:
...
Besides looking at the single predictors vs. the response, we also need to how the predictors relate to each other:
...
-   It is possible that multiple predictors can have an synergestic effect on the outcome; their effect on the outcome occurs jointly - this is called an **Interaction**.
...
```{python}
plt.clf()
ax = sns.regplot(y="Age", x="BMI", data=nhanes, lowess=True, scatter_kws={'alpha':0.1}, line_kws={'color':"r"})
ax.set_xlim([10, 50])
plt.show()
```
...
```{python}
plt.clf()
ax = sns.lmplot(y="MeanBloodPressure", x="BMI", hue="Gender", data=nhanes, lowess=True, scatter_kws={'alpha':0.1})
ax.set(xlim=(10, 50)) 
plt.show()
```
...
where the unknown variables $\beta_0$, $\beta_1$, $\beta_2$, $\beta_3$, called **parameters**, will be learned in the model training process.
...
## Picking a model: Decision Tree Regression?
...
Let's look at our MAE of our model on the test data:
...
mean_absolute_error(y_test_predicted, y_test)
```

Okay, on average our model is off by 8.64 on the scale of $MeanBloodPressure$.
