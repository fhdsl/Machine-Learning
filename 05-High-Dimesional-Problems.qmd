...
Our example dataset for the high-dimensional setting centers around this question: Can we use RNA gene expression a biological response? If so, then we can use RNA gene expression as a biomarker to predict downstream phenotype. We look at bacterial cultures of *Bacillus subtilis* to see whether we can use its gene expression levels to predict its riboflavin (Vitiman A) production.
...
riboflavin_expression = pd.read_csv("classroom_data/riboflavin.csv")
```

The variable `y` corresponds to log-transformed riboflavin (vitamin B2) production rate of Bacillus subtilis. Let's take a look at its distribution.

```{python}
plt.clf()
g = sns.displot(x="y", data=riboflavin_expression)
plt.show()
```

It looks mostly bell-shaped, with a bit of a tail in lower values.
...
```{python}
riboflavin_expression_train, riboflavin_expression_test = train_test_split(riboflavin_expression, test_size=0.2, random_state=1234)
```

We create our `y_train` and `X_train` variables, and when we need to use all predictors, we indicate via the `.` symbol.

```{python}
y_train, X_train = model_matrix("y ~ .", riboflavin_expression_train)
y_test, X_test = model_matrix("y ~ .", riboflavin_expression_test)
```
...
![Source: An Introduction to Statistical Learning, Ch. 6, by Gareth James, Daniela Witten, Trevor Hastie, Roebert Tibshirani, Jonathan Taylor.](images/pca1.png){width="399"}
...
![Source: An Introduction to Statistical Learning, Ch. 6, by Gareth James, Daniela Witten, Trevor Hastie, Roebert Tibshirani, Jonathan Taylor.](images/pca2.png){width="600"}
...
![Source: https://statisticsglobe.com/scree-plot-pca](images/screeplot_pca.png){width="400"}
...
seq = np.linspace(1, len(pca_full.explained_variance_ratio_), len(pca_full.explained_variance_ratio_)) 

plt.clf()
plt.bar(seq[:20], pca_full.explained_variance_ratio_[:20])
plt.ylabel('Fraction of explained variances')
plt.xlabel('Dimensions')
plt.show()

```

Cross Validate our best set of PCs.

```{python}
pc_scores = pca_full.fit_transform(X_train_scaled)
...
for i in range(1, nPC + 1):
    ...
    mean_absolute_errors.append(cv_scores.mean() * -1)

mse_by_n_components = pd.DataFrame(data=dict(components_used=components_used, mean_absolute_errors=mean_absolute_errors))

plt.clf()
plt.plot(np.linspace(1, nPC, nPC), mean_absolute_errors)
...
lin_reg = lin_reg.fit(X_train_pca, np.ravel(y_train))

y_test_predicted = lin_reg.predict(X_test_pca)
